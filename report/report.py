...
class Graphs:
    ...
    # 固件分析综述
    @staticmethod
    def summary_info(content, firmware_name, firmware_md5, firmware_size, pack_id, firmware_file_num, execute_file_num):

        Graphs.draw_con(content, '1 固件分析综述', Graphs.text_type(18, 30, colors.black, 0))
        ...
        content.append(Graphs.draw_text('固件中共有' + str(firmware_file_num) + '个文件', '0'))
        content.append(Graphs.draw_text('固件中共有' + str(execute_file_num) + '个可执行文件', '0'))

        content.append(Spacer(300, 10))  # 添加空白，长度300，宽10

    @staticmethod
    def relation_loophole(content, fw_file_lists, firmware_name, ct):

        Graphs.draw_con(content, '2 组件关联的漏洞', Graphs.text_type(18, 30, colors.black, 0))
            ...
            for file_info in fw_file_lists:
                ...
                if extra_props is not None:
                    ...
                    if len(file_name) > 0 or len(version) > 0 or len(edb_id) > 0:
                        ...
                        row_data = (
                        num, pack_name_text, fw_file_name_text, file_name_text, str(similarity), version, edb_id)
                        num += 1
                        edb_data.append(row_data)

        col_width = [30, 100, 70, 70, 40, 60, 90]

        content.append(Graphs.draw_table(edb_data, col_width))

        if len(edb_data) < 2:
            Graphs.draw_con(content, '组件未关联漏洞', Graphs.text_type(11, 20, colors.black, 1))
        # 关联的漏洞 end

        content.append(Spacer(300, 20))  # 添加空白，长度300，宽10

    @staticmethod
    def fw_file_table(content, fw_file_lists, ct):

        # 添加文档内容标题
        Graphs.draw_con(content, '3 可执行文件详情', Graphs.text_type(18, 30, colors.black, 0))
        ...
        content.append(Graphs.draw_table(file_data, file_col_width))
        # 固件文件列表 end

        content.append(Spacer(300, 20))  # 添加空白，长度300，宽10

    @staticmethod
    def file_inverted_table(content, fw_file_lists, ct):

        Graphs.draw_con(content, '4 特征码', Graphs.text_type(18, 30, colors.black, 0))
        ...
        for fw_file_info in fw_file_lists:
            ...
            if item_list is not None and len(item_list) > 0:
                ...
                # 添加文档内容标题
                Graphs.draw_con(content, ' (' + str(child_num) + ')' + file_name + '文件特征码', Graphs.text_type(15, 30, colors.black, 0))
                ...
                content.append(Graphs.draw_table(data, col_width))

        if child_num == 1:
            Graphs.draw_con(content, '组件文件未生成特征码', Graphs.text_type(11, 20, colors.black, 1))
        # 敏感关键字列表 end
        content.append(Spacer(300, 10))  # 添加空白，长度300，宽10
    ...
    def save(self, pack_id):
        ...
        # 1 固件分析综述
        self.summary_info(content, firmware_name, firmware_md5, firmware_size, pack_id, firmware_file_num, execute_file_num)

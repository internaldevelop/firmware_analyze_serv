...
class JFFS2:
    def __init__(self, image):
        with open(image, 'rb') as self.fd:
            ...
            if (sysstr == "Windows"):
                self.image = mmap.mmap(self.fd.fileno(), 0, access=mmap.ACCESS_READ)
            else:
                self.image = mmap.mmap(self.fd.fileno(), 0, prot=mmap.PROT_READ)  # this is for Unix
    ...
    def scan_dirent(self, mm):
        ...
        pino, version, ino, mctime, nsize, ntype, unused, node_crc, name_crc = struct.unpack(
            "%sLLLLBBHLL" % self.endian, mm[0:28])

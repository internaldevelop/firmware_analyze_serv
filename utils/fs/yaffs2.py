        ...
        while self.offset < self.data_len:
            ...
            if obj_hdr.file_size > 0:
                ...
                if obj_hdr.file_size > (self.data_len - self.offset):
                    raise YAFFSException(
                        "File size for file '%s' exceeds the end of the file system [0x%X]!\n" % (obj_hdr.name,
                                                                                                  obj_hdr.file_size))
            ...
            for entry in parser.next_entry():
                ...
                if Compat.has_key(self.file_paths, entry.parent_obj_id):
                    ...
                    if entry.parent_obj_id != 1:
                        self.dbg_write(
                            "Warning: File %s is the child of an unknown parent object [%d]!\n" % (entry.name,
                                                                                                   entry.parent_obj_id))
                    path = entry.name
...
#         # Make it a bytes array for Python3
#         test_outdir = "c:\\temp"
#         outdir = Compat.str2bytes(test_outdir)
#
#         # 忽略目录文件
#         # # Create directories first, so that files can be written to them
...
#                 entry = fs.file_entries[entry_id]
#                 mode = entry.yst_mode
#                 if int(entry.yaffs_obj_type) == fs.YAFFS_OBJECT_TYPE_FILE:
#                     try:
#                         content = fs.file_entries[entry_id].file_data
#
#                         # with open(file_path, 'wb') as fp:
#                         #     fp.write(fs.file_entries[entry_id].file_data)
#                         # fs._set_mode_owner(file_path, entry)
#                         file_count += 1
#                     except Exception as e:
#                         sys.stderr.write("WARNING: Failed to create file '%s': %s\n" % (file_path, str(e)))
#                 elif int(entry.yaffs_obj_type) == fs.YAFFS_OBJECT_TYPE_SPECIAL:
#                     try:
#                         os.mknod(file_path, entry.yst_mode, entry.yst_rdev)
#                         file_count += 1
#                     except Exception as e:
